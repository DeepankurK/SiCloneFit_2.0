...
//		}
	 ...
	 */
	public static double sampleNewFn(ArrayList<SingleCell> listCells, ArrayList<Clone> listClone,
									 SCSAmplificationErrorLikelihood ampErrLikelihoodObj,
									 double currFp, BetaDistribution fnPriorDist, int nMut){
		
		// Obtain the envelope (Y_max) of the posterior distribution of Fn
		double FnPosteriorMax = getFnPosteriorMax(listCells, listClone, ampErrLikelihoodObj, fnPriorDist, currFp, nMut);
		...
		double x = fn_fX.sample();  // Sample x ~ U(x_min, x_max)
		// calculate f(x), here f is conditional posterior
		double x_posterior = Math.exp(ampErrLikelihoodObj.computeAmpErrorLogLikelihoodCellMatrix(listCells, listClone, currFp, x, nMut)) * fnPriorDist.density(x);
		double y = fn_gY.sample(); // Sample y ~ U(0, Y_max)
		double newFn;
...
//			}
	...
	public static double getFnPosteriorMax(ArrayList<SingleCell> listCells, ArrayList<SingleCell> listCells_tot ,ArrayList<Clone> listClone, 
		...
		double beta = 0.01;
		for (int f = 0; f<99; f++){
			double likelihood = ampErrLikelihoodObj.computeAmpErrorLogLikelihoodCellMatrix(listCells, listCells_tot, listClone, currFp, beta, nMut);
...
//		int accepted = 0;
		while(true){
			//while(true){
			x = fn_fX.sample();
			x_posterior = ampErrLikelihoodObj.computeAmpErrorLogLikelihoodCellMatrix(listCells, listClone, x, currFn, nMut) + Math.log(fpPriorDist.density(x));
...
//			}
	...
	public static double getFpPosteriorMax(ArrayList<SingleCell> listCells,ArrayList<SingleCell> listCells_tot, ArrayList<Clone> listClone, 
		...
		double alpha = 0.01;
		for (int f = 0; f<99; f++){
			double likelihood = ampErrLikelihoodObj.computeAmpErrorLogLikelihoodCellMatrix(listCells, listClone, alpha, currFn, nMut);
