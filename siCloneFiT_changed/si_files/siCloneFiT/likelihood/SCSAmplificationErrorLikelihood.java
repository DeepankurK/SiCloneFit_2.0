...
//    		System.out.println(computeErrorLikelihoodGT(cellCloneCandidateGTVector[i], cellObsGTVector[i]));
    		logProbVector += computeErrorLikelihoodGT(cellCloneCandidateGTVector[i], cellObsGTVector[i], cellObsTotVector[i],_fn.get(i),_fp);    		
    	}
    	return logProbVector;
    }
    public double computeAmpErrorLogLikelihoodCellGTVectorInvidualMutation(Integer[] cellCloneCandidateGTVector,Integer[] cellObsGTVector,Integer[] cellObsTotVector, double fp,double fn,int nMut,int j){
        double logProbVector = 0.0;
        if (fn > 1.0 || fn < 0.0 || fp > 1.0 || fp < 0.0) 
            logProbVector = computeErrorLikelihoodGT(cellCloneCandidateGTVector[j], cellObsGTVector[j],cellObsTotVector[j],_fn.get(j),_fp);
        else
...
//    	}
    ...
    */
	...
	public double computeCloneErrorLogLikelihood(Integer gt, int mutIndex, Clone clone,HashMap<Integer, Integer[]> cellGTVectorMap, HashMap<Integer, Integer[]> cellTotVectorMap){
		double logLikelihood = 0.0;
		for (Integer cellID: clone.memberCellList){
			Integer obsCellGT = cellGTVectorMap.get(cellID)[mutIndex];
			Integer TotCellGT = cellTotVectorMap.get(cellID)[mutIndex];
			double thisMutLikelihood = this.computeErrorLikelihoodGT(gt, obsCellGT,TotCellGT,_fn.get(mutIndex),_fp);
